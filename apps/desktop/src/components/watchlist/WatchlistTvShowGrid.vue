<template>
  <div class="movie-grid-container">
    <!-- Skeleton loaders -->
    <div v-if="isLoadingTvShows" class="movie-grid">
    ...
    <!-- Empty state -->
    <div v-if="!isLoadingTvShows && filteredTvShows.length === 0" class="empty-state">
...
const removeFromWatchlist = async (show: any) => {
  try {
    ...
    console.error('Failed to remove TV show from watchlist:', err);
    error('Watchlist Error', 'Failed to remove TV show from your watchlist. Please try again.');
  }
};
...
const filteredTvShows = computed(() => {
  if (!props.searchQuery) {
    return watchlistTvShows.value;
  }
  return watchlistTvShows.value.filter(show =>
    show.name.toLowerCase().includes(props.searchQuery.toLowerCase())
  );
});
</script>
